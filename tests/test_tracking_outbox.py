...
def test_drain_replays_pending_practice_status_after_restart(tmp_path):
    first = TrackingOutbox(tmp_path / "outbox.json")
    first.enqueue_practice_session_status("p1", "ended")
    api = MagicMock()
    api.patch_practice_session_status.side_effect = ApiError(status=0, code="NETWORK", message="down")

    first.drain(api)

    restarted = TrackingOutbox(tmp_path / "outbox.json")
    api.patch_practice_session_status.side_effect = None
    api.patch_practice_session_status.reset_mock()
    restarted.drain(api, force=True)
